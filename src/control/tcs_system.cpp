...
static float calculateVehicleSpeed(const float wheelSpeeds[4]) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (!std::isfinite(wheelSpeeds[i]) || wheelSpeeds[i] < 0.0f) {
      Logger::warnf("TCS: Invalid wheel speed %.2f on wheel %d", wheelSpeeds[i],
                    i);
      continue;
    }
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (slip > config.slipThreshold) {
      if (!state.wheels[i].active) {
        ...
        state.wheels[i].powerReduction =
            clampf(state.wheels[i].powerReduction, 0.0f, 80.0f);
      }
      ...
      if (state.wheels[i].powerReduction > 0.0f) {
        ...
        state.wheels[i].powerReduction =
            clampf(state.wheels[i].powerReduction, 0.0f, 100.0f);
      }
...
bool isActive() { return state.systemActive; }
...
bool TCSSystem::initOK() { return initialized; }
