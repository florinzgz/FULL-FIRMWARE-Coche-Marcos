...
void update() {
    ...
    if (pedalState.valid) {
        childPedalLimit = pedalState.percent;
    }
    ...
    if (frontDist == ObstacleConfig::DISTANCE_INVALID || frontDist > 4000) {
        ...
        if (status.vehicleDetected) {
            ...
            if (targetLostMs == 0) {
                targetLostMs = now;
                Logger::debug("ACC: Target lost, starting timeout");
    ...
    // v2.12.0: Debug logging at 10Hz
    if (now % 1000 < 100) {  // Log once per second
        Logger::debugf("ACC: dist=%dmm, target=%dmm, adj=%.2f, pedal=%.1f%%", 
                      frontDist, config.targetDistanceMm, adjustment, childPedalLimit);
    }
}
