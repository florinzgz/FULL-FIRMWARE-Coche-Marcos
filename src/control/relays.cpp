...
static const unsigned long RELAY_DEBOUNCE_MS =
    50; // Debounce para cambios de estado
...
void Relays::enablePower() {
  ...
  if (System::hasError()) {
    Logger::errorf(
        "Relays: no se puede activar con errores del sistema activos");
    System::logError(601);
    return;
  }
...
void Relays::setLights(bool on) {
  ...
  state.lightsOn = on;
  Logger::infof("Relays lights %s (GPIO %d)", on ? "ON" : "OFF",
                PIN_RELAY_SPARE);
}
...
void Relays::setMedia(bool on) {
  ...
  state.mediaOn = on;
  Logger::infof("Relays media %s (GPIO %d)", on ? "ON" : "OFF",
                PIN_RELAY_MEDIA);
#else
  state.mediaOn = on;
...
void Relays::update() {
  ...
  if (seqState != SEQ_IDLE && (now - seqStepStartMs > SEQUENCE_TIMEOUT_MS)) {
    Logger::errorf("Relays: secuencia timeout estado %d - forzando apagado",
                   (int)seqState);
    System::logError(650);
    digitalWrite(PIN_RELAY_DIR, LOW);
    digitalWrite(PIN_RELAY_TRAC, LOW);
  ...
  if (!isnan(batteryCurrent) && batteryCurrent > BATTERY_OVERCURRENT_LIMIT_A &&
      batteryCurrent < 10000.0f) {
    Logger::errorf("Relays: OVERCURRENT batería %.1fA (límite %.1fA)",
                   batteryCurrent, BATTERY_OVERCURRENT_LIMIT_A);
    System::logError(602);
    system_error = true;
  }
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (!isnan(temp) && temp > MOTOR_OVERTEMP_LIMIT_C && temp < 1000.0f) {
      Logger::errorf("Relays: OVERTEMP motor %d: %.1f°C (límite %.1f°C)", i,
                     temp, MOTOR_OVERTEMP_LIMIT_C);
  ...
  if (!isnan(batteryVoltage) && batteryVoltage > 0.0f) {
    if (batteryVoltage < BATTERY_VOLTAGE_MIN_V) {
      Logger::errorf("Relays: BATERÍA BAJA %.1fV (crítico <%.1fV)",
                     batteryVoltage, BATTERY_VOLTAGE_MIN_V);
      System::logError(607);
      system_error = true;
    } else if (batteryVoltage > BATTERY_VOLTAGE_MAX_V &&
               batteryVoltage < 1000.0f) {
      Logger::errorf("Relays: BATERÍA ALTA %.1fV (peligro >%.1fV)",
                     batteryVoltage, BATTERY_VOLTAGE_MAX_V);
      System::logError(608);
      system_error = true;
    }
  }
  ...
  if (system_error) {
    ...
    if (consecutiveErrors >= 3) {
      Logger::errorf("Relays: ERROR CRÍTICO (%d consecutivos) - Iniciando "
    ...
    if (now - lastErrorMs > 1000) { consecutiveErrors = 0; }
  }
}

const Relays::State &Relays::get() { return state; }

bool Relays::initOK() { return initialized; }