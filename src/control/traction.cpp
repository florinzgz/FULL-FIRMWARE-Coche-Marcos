...
void Traction::setDemand(float pedalPct) {
  ...
  if (std::fabs(pedalPct - rampedDemand) > 1.0f) {
...
void Traction::update() {
  ...
  // 🔒 FIX: Detect reverse gear from shifter (except in axis rotation mode)
  ...
  // 🔒 FIX: Apply gear-based power limiting
