...
static constexpr uint32_t TEST_COLOR_DELAY_MS =
    500;                                          // Delay between color screens
...
static constexpr uint32_t WATCHDOG_FEED_INTERVAL_MS =
    1000; // Watchdog feed interval

// Color test palette
static const uint16_t TEST_COLORS[] = {TFT_RED,    TFT_GREEN,  TFT_BLUE,
                                       TFT_YELLOW, TFT_CYAN,   TFT_MAGENTA,
                                       TFT_WHITE,  TFT_ORANGE, TFT_BLACK};
static constexpr size_t NUM_TEST_COLORS =
...
void loopDisplayTest() {
  ...
  yield(); // Keep watchdog happy (Arduino framework handles it)
}
