...
// Acumuladores para promedios (a nivel de namespace para evitar variables
...
void update() {
  ...
  // Guardado periódico
  if (cfg.persistToStorage &&
      (now - lastPersistMs) > (cfg.persistIntervalSec * 1000UL)) {
    saveToStorage();
    lastPersistMs = now;
  }
}

const VehicleData &getData() { return data; }
...
void saveToStorage() {
  ...
  if (written == sizeof(VehicleData)) {
    ...
    Logger::errorf("Telemetry: Error guardando datos (%u/%u bytes)", written,
                   sizeof(VehicleData));
  }
}
...
void loadFromStorage() {
  ...
  if (len != sizeof(VehicleData)) {
    Logger::warnf("Telemetry: Tamaño datos inválido (%u vs %u)", len,
                  sizeof(VehicleData));
    prefs.end();
    data = VehicleData();
    saveToStorage();
  ...
  if (isCorrupted(temp)) {
    ...
    data = temp;
    Logger::infof("Telemetry: Cargado %.1f km, %lu horas", data.totalDistanceKm,
                  static_cast<unsigned long>(data.runtimeSeconds / 3600));
  }
}

void setEnabled(bool enable) { cfg.enabled = enable; }

bool initOK() { return initialized; }

float getAvgConsumptionWhKm() {
  if (data.sessionDistanceKm <= MIN_DISTANCE_THRESHOLD) return 0.0f;
  return static_cast<float>((data.sessionEnergyKwh * 1000.0) /
                            data.sessionDistanceKm);
}
...
float getEstimatedRangeKm() {
  ...
  float remainingKwh =
      cfg.batteryCapacityKwh * (data.stateOfChargePercent / 100.0f);
...
String exportToJson() {
  ...
  json += "\"runtimeHours\":" +
          String(static_cast<unsigned long>(data.runtimeSeconds / 3600)) + ",";
...
void updateSpeed(float speedKmh) {
  ...
  if (speedKmh > data.maxSpeedKmh) { data.maxSpeedKmh = speedKmh; }
...
void updateBattery(float voltage, float current, float soc) {
  ...
  if (voltage > 0) {
    ...
    if (voltage > data.maxBatteryVoltage) { data.maxBatteryVoltage = voltage; }
  }
...
void updateTemperature(float motorTemp, float batteryTemp) {
  if (motorTemp > data.maxMotorTemp) { data.maxMotorTemp = motorTemp; }
  ...
  if (batteryTemp > data.maxBatteryTemp) { data.maxBatteryTemp = batteryTemp; }
}
