...
#error                                                                         \
    "I2C_FREQUENCY must be defined via build flags (e.g., -DI2C_FREQUENCY=400000)"
#endif
...
static constexpr uint32_t RECOVERY_FREQUENCY =
...
bool recoverBus() {
  ...
  Logger::infof("Bus stuck - SDA:%d SCL:%d", digitalRead(pinSDA),
                digitalRead(pinSCL));
  ...
  bool recovered =
      (digitalRead(pinSDA) == HIGH) && (digitalRead(pinSCL) == HIGH);
...
bool readBytesWithRetry(uint8_t deviceAddr, uint8_t regAddr, uint8_t *buffer,
  ...
  if ((now - dev.lastSuccessMs) > SKIP_SENSOR_AFTER_MS) {
    if (dev.online) {
      dev.online = false;
      Logger::warn(
          "I2CRecovery: Dispositivo marcado OFFLINE (1 min sin respuesta)");
    }
  ...
  for (uint8_t retry = 0; retry <= MAX_RETRIES; retry++) {
    if (retry > 0) {
      Logger::infof("I2C retry %d/%d (dev 0x%02X)", retry, MAX_RETRIES,
                    deviceAddr);
    ...
    if (result != 0) {
      if ((millis() - startMs) > TIMEOUT_MS) {
        Serial.printf(
            "[I2CRecovery] ERROR: I2C timeout escribiendo reg (dev 0x%02X)\n",
            deviceAddr);
        continue; // Retry
      }
      continue;
  ...
  uint8_t exponent =
      (dev.consecutiveFailures - 1 > 5) ? 5 : (dev.consecutiveFailures - 1);
  uint32_t backoff = 1000 * (1 << exponent);
  if (backoff > MAX_BACKOFF_MS) backoff = MAX_BACKOFF_MS;
  dev.nextRetryMs = now + backoff;

  Logger::errorf("I2C fallo dev 0x%02X (fallos:%d, backoff:%lus)", deviceAddr,
                 dev.consecutiveFailures, backoff / 1000);
...
bool writeBytesWithRetry(uint8_t deviceAddr, uint8_t regAddr,
  ...
  // Verificar backoff
  if (now < dev.nextRetryMs) { return false; }
  ...
  Logger::errorf("I2C write fallo dev 0x%02X (backoff:%lus)", deviceAddr,
                 backoff / 1000);

  if (dev.consecutiveFailures >= 3) { recoverBus(); }
...
bool reinitSensor(uint8_t channel, uint8_t deviceAddr, uint8_t deviceId) {
  ...
  if (!tcaSelectSafe(channel)) {
    ...
    if (!recoverBus()) { return false; }

    // Retry select tras recovery
    if (!tcaSelectSafe(channel)) { return false; }
  }
  ...
  if (result == 0) {
    ...
    // Resetear estado
    if (deviceId < MAX_DEVICES) { resetDeviceState(deviceId); }
    return true;
  }
...
void markDeviceOffline(uint8_t deviceId) {
  ...
  uint8_t exponent = (devices[deviceId].consecutiveFailures - 1 > 5)
                         ? 5
                         : (devices[deviceId].consecutiveFailures - 1);
  uint32_t backoff = 1000 * (1 << exponent);
  if (backoff > MAX_BACKOFF_MS) backoff = MAX_BACKOFF_MS;
