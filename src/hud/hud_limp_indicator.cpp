...
static uint16_t getColorForState(LimpMode::LimpState state) {
  switch (state) {
    case LimpMode::LimpState::NORMAL:    return COLOR_NORMAL;
    case LimpMode::LimpState::DEGRADED:  return COLOR_DEGRADED;
    case LimpMode::LimpState::LIMP:      return COLOR_LIMP;
    case LimpMode::LimpState::CRITICAL:  return COLOR_CRITICAL;
    default:                              return COLOR_CRITICAL;
  }
}
...
static const char* getTextForState(LimpMode::LimpState state) {
  switch (state) {
    case LimpMode::LimpState::NORMAL:    return nullptr;  // Don't show
    case LimpMode::LimpState::DEGRADED:  return "DEG";
    case LimpMode::LimpState::LIMP:      return "LIMP";
    case LimpMode::LimpState::CRITICAL:  return "SAFE";
    default:                              return "ERR";
  }
}
...
static void drawIndicator(LimpMode::LimpState state) {
  ...
  tft->drawRect(INDICATOR_X, INDICATOR_Y, INDICATOR_WIDTH, INDICATOR_HEIGHT, 
                color);
  tft->drawRect(INDICATOR_X + 1, INDICATOR_Y + 1, 
                INDICATOR_WIDTH - 2, INDICATOR_HEIGHT - 2, color);
