...
MenuEncoderCalibration::CalibrationStep MenuEncoderCalibration::currentStep =
    STEP_CENTER;
...
MenuEncoderCalibration::ButtonBounds MenuEncoderCalibration::btnSetCenter = {
    20, 180, 100, 35};
MenuEncoderCalibration::ButtonBounds MenuEncoderCalibration::btnSetLeft = {
    130, 180, 100, 35};
MenuEncoderCalibration::ButtonBounds MenuEncoderCalibration::btnSetRight = {
    240, 180, 100, 35};
MenuEncoderCalibration::ButtonBounds MenuEncoderCalibration::btnSave = {
    350, 180, 110, 35};
MenuEncoderCalibration::ButtonBounds MenuEncoderCalibration::btnReset = {
    20, 230, 150, 35};
MenuEncoderCalibration::ButtonBounds MenuEncoderCalibration::btnBack = {
    310, 230, 150, 35};
...
void MenuEncoderCalibration::handleTouch(uint16_t x, uint16_t y) {
  ...
  // Check Save button
  if (x >= btnSave.x && x <= btnSave.x + btnSave.w && y >= btnSave.y &&
      y <= btnSave.y + btnSave.h) {
    handleSave();
    return;
  }

  // Check Reset button
  if (x >= btnReset.x && x <= btnReset.x + btnReset.w && y >= btnReset.y &&
      y <= btnReset.y + btnReset.h) {
    handleReset();
    return;
  }

  // Check Back button - handled externally
  if (x >= btnBack.x && x <= btnBack.x + btnBack.w && y >= btnBack.y &&
      y <= btnBack.y + btnBack.h) {
    handleBack();
    return;
  }
}
...
void MenuEncoderCalibration::drawHeader() {
  ...
  SafeDraw::drawString(ctx, "ENCODER CALIBRATION", 240, 10, 4);

  SafeDraw::drawLine(ctx, 20, 40, 460, 40, TFT_DARKGREY);
}
...
void MenuEncoderCalibration::drawLiveValue() {
  ...
  SafeDraw::drawString(ctx, "Live:", 155, 60, 2);
  ...
  if (currentStep == STEP_CENTER) {
    valueColor =
        (abs(liveEncoderValue - tempCenter) < 10) ? COLOR_ACTIVE : COLOR_TEXT;
  } else if (currentStep == STEP_LEFT) {
    valueColor =
        (liveEncoderValue < tempCenter - 100) ? COLOR_ACTIVE : COLOR_WARNING;
  ...
  SafeDraw::drawString(ctx, valueStr, 165, 55, 4);
}
...
void MenuEncoderCalibration::drawCalibrationSteps() {
  ...
  drawStepIndicator(STEP_CENTER, "Center", tempCenter,
                    currentStep == STEP_CENTER);

  tft->setTextDatum(TL_DATUM);
  y += stepSpacing;
  drawStepIndicator(STEP_LEFT, "Left Limit", tempLeftLimit,
                    currentStep == STEP_LEFT);

  y += stepSpacing;
  drawStepIndicator(STEP_RIGHT, "Right Limit", tempRightLimit,
                    currentStep == STEP_RIGHT);
}

void MenuEncoderCalibration::drawStepIndicator(uint8_t step, const char *label,
                                               int32_t value, bool active) {
  int y = 100 + step * 25;
  int iconX = 30;
  int labelX = 55;
  int valueX = 200;
  ...
  SafeDraw::fillCircle(ctx, iconX, y + 6, 8, iconColor);
  if (complete) {
    // Draw checkmark
    SafeDraw::drawLine(ctx, iconX - 4, y + 6, iconX - 1, y + 9, COLOR_BG);
    SafeDraw::drawLine(ctx, iconX - 1, y + 9, iconX + 5, y + 3, COLOR_BG);
  } else if (active) {
    // Draw dot
    SafeDraw::fillCircle(ctx, iconX, y + 6, 3, COLOR_BG);
  }

  // Step label
  tft->setTextDatum(TL_DATUM);
  tft->setTextColor(active ? COLOR_ACTIVE : COLOR_TEXT, COLOR_BG);
  SafeDraw::drawString(ctx, label, labelX, y, 2);

  // Step value
  char valueStr[16];
  snprintf(valueStr, sizeof(valueStr), "%ld", value);
  tft->setTextDatum(TR_DATUM);
  tft->setTextColor(complete ? COLOR_COMPLETE
                             : (active ? COLOR_ACTIVE : COLOR_INACTIVE),
                    COLOR_BG);
  SafeDraw::drawString(ctx, valueStr, valueX, y, 2);
}

void MenuEncoderCalibration::drawButtons() {
  // Set Center button
  uint16_t centerColor =
      (currentStep == STEP_CENTER) ? COLOR_ACTIVE : COLOR_INACTIVE;
  tft->fillRoundRect(btnSetCenter.x, btnSetCenter.y, btnSetCenter.w,
                     btnSetCenter.h, 5, centerColor);
  ...
  // Set Left button
  uint16_t leftColor =
      (currentStep == STEP_LEFT) ? COLOR_WARNING : COLOR_INACTIVE;
  tft->fillRoundRect(btnSetLeft.x, btnSetLeft.y, btnSetLeft.w, btnSetLeft.h, 5,
                     leftColor);
  ...
  // Set Right button
  uint16_t rightColor =
      (currentStep == STEP_RIGHT) ? COLOR_WARNING : COLOR_INACTIVE;
  tft->fillRoundRect(btnSetRight.x, btnSetRight.y, btnSetRight.w, btnSetRight.h,
                     5, rightColor);
...
void MenuEncoderCalibration::drawInstructions() {
  ...
  SafeDraw::drawString(ctx, getStepInstruction(), 240, 295, 2);
}
...
void MenuEncoderCalibration::drawVisualIndicator() {
  ...
  tft->fillRect(centerX + 2, barY + 25, barX + barW - centerX - 4, 10,
                COLOR_WARNING);
...
void MenuEncoderCalibration::handleSetCenter() {
  ...
  if (tempCenter < -2000 || tempCenter > 4000) {
    Logger::warnf(
        "Encoder center value unusual: %ld (expected range -2000 to 4000)",
        tempCenter);
  }
}
...
void MenuEncoderCalibration::handleSetLeft() {
  ...
  if (liveEncoderValue >= tempCenter) {
    Logger::errorf("Invalid left limit: %ld >= center %ld. Please turn wheel "
                   "LEFT from center.",
...
void MenuEncoderCalibration::handleSetRight() {
  ...
  if (liveEncoderValue <= tempCenter) {
    Logger::errorf("Invalid right limit: %ld <= center %ld. Please turn wheel "
                   "RIGHT from center.",
                   liveEncoderValue, tempCenter);
    Alerts::play(Audio::AUDIO_ENCODER_ERROR);
    return; // Don't advance step or set invalid value
...
void MenuEncoderCalibration::saveCalibration() {
  ...
  if (totalRange < 200) {
    Logger::warnf("Calibration range too small: %ld (expected >200)",
                  totalRange);
  }

  // Ensure values fit into int16_t before saving to config to avoid truncation
  if (tempCenter < -32768 || tempCenter > 32767 || tempLeftLimit < -32768 ||
      tempLeftLimit > 32767 || tempRightLimit < -32768 ||
      tempRightLimit > 32767) {
    Logger::errorf("Calibration values out of int16_t range: left=%ld, "
                   "center=%ld, right=%ld",
                   tempLeftLimit, tempCenter, tempRightLimit);
    Alerts::play(Audio::AUDIO_ENCODER_ERROR);
    return;
