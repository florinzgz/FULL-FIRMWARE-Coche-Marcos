...
bool LEDControlMenu::handleTouch(uint16_t x, uint16_t y) {
    ...
    if (handlePatternButtonTouch(x, y)) {
        needsRedraw = true;
    }
    
    // Check brightness slider
    if (handleBrightnessTouch(x, y)) {
        needsRedraw = true;
    }
    
    // Check speed slider
    if (handleSpeedTouch(x, y)) {
        needsRedraw = true;
    }
    
    // Check color picker
    if (handleColorPickerTouch(x, y)) {
        needsRedraw = true;
    }
...
void LEDControlMenu::drawBrightnessSlider() {
    ...
    tft.drawString(valueStr, SLIDER_X + SLIDER_W/2, BRIGHTNESS_Y + SLIDER_H/2, 2);
}
...
void LEDControlMenu::drawBackButton() {
    tft.fillRoundRect(BACK_BTN_X, BACK_BTN_Y, BACK_BTN_W, BACK_BTN_H, 5, TFT_DARKGREY);
    tft.drawRoundRect(BACK_BTN_X, BACK_BTN_Y, BACK_BTN_W, BACK_BTN_H, 5, COLOR_TEXT);
    
    tft.setTextDatum(MC_DATUM);
    tft.setTextColor(COLOR_TEXT, TFT_DARKGREY);
    tft.drawString("BACK", BACK_BTN_X + BACK_BTN_W/2, BACK_BTN_Y + BACK_BTN_H/2, 2);
