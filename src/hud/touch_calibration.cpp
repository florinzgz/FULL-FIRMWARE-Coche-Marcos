...
// Reduced from 10 samples @ 50ms = 500ms to 5 samples @ 10ms = 50ms (10×
// faster)
...
static const uint32_t TOUCH_RELEASE_WAIT =
    200; // ms to wait for touch release (was 500)
...
bool update() {
  ...
  case CalibrationState::Point1: {
    ...
    if (collectTouchSample(point1_rawX, point1_rawY)) {
      ...
      SafeDraw::drawString(ctx, "Touch the RED target", TouchConstants::SCREEN_WIDTH / 2,
                      10, 2);
      SafeDraw::drawString(ctx, "Point 2 of 2", TouchConstants::SCREEN_WIDTH / 2, 30, 2);
...
CalibrationState getState() { return state; }
...
CalibrationResult getResult() { return result; }
...
static void drawCalibrationPoint(int x, int y, uint16_t color) {
  ...
  SafeDraw::drawCircle(ctx, x, y, CALIB_RADIUS + 5, TFT_WHITE);
  SafeDraw::drawLine(ctx, x - 20, y, x - 10, y, TFT_WHITE);
  SafeDraw::drawLine(ctx, x + 10, y, x + 20, y, TFT_WHITE);
  SafeDraw::drawLine(ctx, x, y - 20, x, y - 10, TFT_WHITE);
  SafeDraw::drawLine(ctx, x, y + 10, x, y + 20, TFT_WHITE);
}
...
static void drawInstructions() {
  ...
  SafeDraw::drawString(ctx, "TOUCH CALIBRATION", TouchConstants::SCREEN_WIDTH / 2, 60, 4);

  tft->setTextColor(TFT_WHITE, TFT_BLACK);
  tft->setTextDatum(TL_DATUM);

  int y = 120;
  SafeDraw::drawString(ctx, "You will be asked to touch", 60, y, 2);
  y += 25;
  SafeDraw::drawString(ctx, "2 points on the screen.", 60, y, 2);
  y += 25;
  y += 25; // Blank line for spacing
  SafeDraw::drawString(ctx, "Touch accurately for best", 60, y, 2);
  y += 25;
  SafeDraw::drawString(ctx, "results.", 60, y, 2);
  y += 30;

  tft->setTextColor(TFT_GREEN, TFT_BLACK);
  tft->setTextDatum(TC_DATUM);
  SafeDraw::drawString(ctx, "Touch anywhere to start", TouchConstants::SCREEN_WIDTH / 2,
                  260, 2);
}
  ...
  if (tft->getTouch(&tx, &ty)) {
    if (now - lastSampleTime >= SAMPLE_INTERVAL) {
      ...
      if (samplesCollected >= SAMPLE_COUNT) {
        ...
        // Reject samples that are clearly invalid (hardware issue or weak
        // touch) Valid range is typically 100-4000 for XPT2046
        if (avgX < TOUCH_SAMPLE_MIN || avgX > TOUCH_SAMPLE_MAX ||
            avgY < TOUCH_SAMPLE_MIN || avgY > TOUCH_SAMPLE_MAX) {
          Logger::warnf(
              "TouchCalibration: Sample out of expected range X=%d Y=%d", avgX,
              avgY);
          Logger::warn(
              "TouchCalibration: Touch harder or check hardware connection");
          Logger::warn("TouchCalibration: Resetting sample collection, please "
                       "try again");
          ...
          int targetX = (state == CalibrationState::Point1)
                            ? CALIB_MARGIN
                            : (TouchConstants::SCREEN_WIDTH - CALIB_MARGIN);
          int targetY = (state == CalibrationState::Point1)
                            ? CALIB_MARGIN
                            : (TouchConstants::SCREEN_HEIGHT - CALIB_MARGIN);
...
static void calculateCalibration() {
  ...
  // Point 2: Bottom-right (SCREEN_WIDTH - CALIB_MARGIN, SCREEN_HEIGHT -
  // CALIB_MARGIN)
  ...
  // So we need to extrapolate to the actual edges (0, 0) and (SCREEN_WIDTH,
  // SCREEN_HEIGHT)
  float scaleX = (float)(TouchConstants::SCREEN_WIDTH) /
                 (float)(TouchConstants::SCREEN_WIDTH - 2 * CALIB_MARGIN);
  float scaleY = (float)(TouchConstants::SCREEN_HEIGHT) /
                 (float)(TouchConstants::SCREEN_HEIGHT - 2 * CALIB_MARGIN);
  ...
  maxX = ((uint32_t)maxX + (uint32_t)extrapolateX < MAX_TOUCH_VALUE)
             ? (maxX + extrapolateX)
             : MAX_TOUCH_VALUE;
  minY = ((uint32_t)minY > (uint32_t)extrapolateY) ? (minY - extrapolateY) : 0;
  maxY = ((uint32_t)maxY + (uint32_t)extrapolateY < MAX_TOUCH_VALUE)
             ? (maxY + extrapolateY)
             : MAX_TOUCH_VALUE;
...
static void drawVerification() {
  ...
  SafeDraw::drawString(ctx, "CALIBRATION COMPLETE!", TouchConstants::SCREEN_WIDTH / 2,
                  100, 4);
  ...
  int y = 160;
  SafeDraw::drawString(ctx, "Calibration values:", 60, y, 2);
  y += 25;

  char buf[64];
  snprintf(buf, sizeof(buf), "Min X: %d", result.calibData[0]);
  SafeDraw::drawString(ctx, buf, 60, y, 2);
  y += 20;
  ...
  tft->setTextDatum(TC_DATUM);
  SafeDraw::drawString(ctx, "Saving calibration...", TouchConstants::SCREEN_WIDTH / 2,
                  280, 2);
