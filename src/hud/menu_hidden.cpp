...
// 🔒 CRITICAL: Helper function to ensure displayBrightness is never corrupted
// before saving Stack overflow or memory corruption could set brightness to 0,
// causing black screen This function validates brightness before every save to
// prevent permanent corruption Note: Only validates displayBrightness. Other
// fields are assumed correct from calibration functions.
static void safeSaveConfig() {
  ...
  if (cfg.displayBrightness < 1 || cfg.displayBrightness > 255) {
    Logger::warnf("MenuHidden: displayBrightness corrupted (%d), restoring to "
                  "default (%d)",
...
static bool regenAdjustFirstCall =
    true; // 🔒 v2.10.0: Track first draw for regen adjust
static bool calibrationFirstCall =
    true; // 🔒 v2.10.0: Track first draw for pedal/encoder calibration
...
// Constantes de debounce y timing (touch calibration centralizada en
// touch_map.h)
static const uint32_t DEBOUNCE_TIMEOUT_MS = 500; // Timeout para debounce
static const uint32_t DEBOUNCE_SHORT_MS =
    200; // ✅ v2.7.0: Debounce corto para ajustes rápidos
static const uint32_t FEEDBACK_DISPLAY_MS =
    1500; // ✅ v2.7.0: Tiempo de visualización de feedback
...
static const char *const MENU_ITEMS[NUM_MENU_ITEMS] = {
    "1) Calibrar pedal",    "2) Calibrar encoder",
    "3) Calibrar touch", // 🔒 v2.9.0: Nueva opción
    "4) Ajuste regen (%)",  "5) Modulos/Sensores", "6) Guardar y salir",
    "7) Restaurar fabrica", "8) Ver errores",      "9) Borrar errores"};
...
static void drawCalibrationScreen(const char *title, const char *instruction,
                                  const char *value) {
...
static void startPedalCalibration() {
  ...
  pedalCalibMin = 0;
  pedalCalibMax = 4095;
  calibrationFirstCall =
      true; // 🔒 v2.10.0: Reset for full screen clear on first draw
...
static void startTouchCalibration() {
  ...
  Alerts::play({Audio::AUDIO_CAL_ENCODER,
                Audio::Priority::PRIO_HIGH}); // Reuse encoder sound
...
static void updatePedalCalibration(bool touched) {
  ...
  if (calibState == CalibrationState::PEDAL_MIN) {
    ...
    drawCalibrationScreen("CALIBRAR PEDAL", "Suelta el pedal (mínimo)",
                          valueStr);

    // Actualizar mínimo continuamente
    if (pedal.raw > 0) { pedalCalibMin = pedal.raw; }
    ...
    snprintf(valueStr, sizeof(valueStr), "ADC: %d", pedal.raw);
    drawCalibrationScreen("CALIBRAR PEDAL", "Pisa el pedal al máximo",
                          valueStr);

    // Actualizar máximo continuamente
    if (pedal.raw > pedalCalibMin + 100) { pedalCalibMax = pedal.raw; }
    ...
    if (pedalCalibMax >= pedalCalibMin + 500) {
      ...
      Logger::infof("Calibración pedal guardada: %d - %d", pedalCalibMin,
                    pedalCalibMax);
...
static void updateEncoderCalibration(bool touched) {
  ...
  if (calibState == CalibrationState::ENCODER_CENTER) {
    ...
    drawCalibrationScreen("CALIBRAR VOLANTE", "Centra el volante y confirma",
                          valueStr);
...
static void updateRegenAdjust(int touchX, int touchY, bool touched) {
  ...
  if (needsRedraw) {
    waitTouchRelease(
        DEBOUNCE_SHORT_MS); // Debounce corto para permitir ajuste rápido
    drawRegenAdjustScreen();
  }
...
static void drawModulesConfigScreen() {
  ...
  auto drawToggle = [](int x, int y, int w, int h, const char *label,
                       bool enabled) {
  ...
  drawToggle(col1X, startY, btnW, btnH, label,
             tempModulesState.tractionEnabled);

  makeLabel(label, sizeof(label), "INA226",
            tempModulesState.currentSensorsEnabled);
  drawToggle(col1X, startY + rowSpacing, btnW, btnH, label,
             tempModulesState.currentSensorsEnabled);

  makeLabel(label, sizeof(label), "TEMP", tempModulesState.tempSensorsEnabled);
  drawToggle(col1X, startY + 2 * rowSpacing, btnW, btnH, label,
             tempModulesState.tempSensorsEnabled);

  makeLabel(label, sizeof(label), "RUEDAS",
            tempModulesState.wheelSensorsEnabled);
  drawToggle(col2X, startY, btnW, btnH, label,
             tempModulesState.wheelSensorsEnabled);
  ...
  tft->setTextColor(TFT_YELLOW, TFT_BLACK);
  SafeDraw::drawString(ctx, "Toca para activar/desactivar módulos y sensores",
                       240, 300, 1);
}
...
static void updateModulesConfig(int touchX, int touchY, bool touched) {
  ...
  auto inButton = [](int touchXCoord, int touchYCoord, int buttonX, int buttonY,
  ...
  if (inButton(touchX, touchY, col1X, startY, btnW, btnH)) {
    ...
    Logger::infof("Tracción: %s",
                  tempModulesState.tractionEnabled ? "ON" : "OFF");
    Alerts::play({Audio::AUDIO_MODULO_OK, Audio::Priority::PRIO_NORMAL});
    stateChanged = true;
  } else if (inButton(touchX, touchY, col1X, startY + rowSpacing, btnW, btnH)) {
    tempModulesState.currentSensorsEnabled =
        !tempModulesState.currentSensorsEnabled;
    Logger::infof("Sensores INA226: %s",
                  tempModulesState.currentSensorsEnabled ? "ON" : "OFF");
    Alerts::play({Audio::AUDIO_MODULO_OK, Audio::Priority::PRIO_NORMAL});
    stateChanged = true;
  } else if (inButton(touchX, touchY, col1X, startY + 2 * rowSpacing, btnW,
                      btnH)) {
    tempModulesState.tempSensorsEnabled = !tempModulesState.tempSensorsEnabled;
    Logger::infof("Sensores temperatura: %s",
                  tempModulesState.tempSensorsEnabled ? "ON" : "OFF");
    Alerts::play({Audio::AUDIO_MODULO_OK, Audio::Priority::PRIO_NORMAL});
    stateChanged = true;
  } else if (inButton(touchX, touchY, col2X, startY, btnW, btnH)) {
    tempModulesState.wheelSensorsEnabled =
        !tempModulesState.wheelSensorsEnabled;
    Logger::infof("Sensores rueda: %s",
                  tempModulesState.wheelSensorsEnabled ? "ON" : "OFF");
    Alerts::play({Audio::AUDIO_MODULO_OK, Audio::Priority::PRIO_NORMAL});
    stateChanged = true;
  }
  ...
  if (stateChanged) { drawModulesConfigScreen(); }
}
...
static const int MAX_DISPLAYED_ERRORS =
    7; // Máximo de errores mostrados a la vez
static const int ERROR_LINE_LENGTH_THRESHOLD =
    40; // Umbral para usar fuente pequeña
...
static int getTouchedMenuOption(int touchX, int touchY) {
  ...
  if (touchY < MENU_Y1 || touchY > MENU_Y1 + NUM_MENU_ITEMS * MENU_ITEM_HEIGHT)
    return -1;
...
static const KeypadButton keypadButtons[12] = {
    ...
    {KEYPAD_X + (KEYPAD_BTN_WIDTH + KEYPAD_SPACING),
     KEYPAD_Y + (KEYPAD_BTN_HEIGHT + KEYPAD_SPACING), 5, "5"},
    {KEYPAD_X + 2 * (KEYPAD_BTN_WIDTH + KEYPAD_SPACING),
     KEYPAD_Y + (KEYPAD_BTN_HEIGHT + KEYPAD_SPACING), 6, "6"},

    {KEYPAD_X, KEYPAD_Y + 2 * (KEYPAD_BTN_HEIGHT + KEYPAD_SPACING), 7, "7"},
    {KEYPAD_X + (KEYPAD_BTN_WIDTH + KEYPAD_SPACING),
     KEYPAD_Y + 2 * (KEYPAD_BTN_HEIGHT + KEYPAD_SPACING), 8, "8"},
    {KEYPAD_X + 2 * (KEYPAD_BTN_WIDTH + KEYPAD_SPACING),
     KEYPAD_Y + 2 * (KEYPAD_BTN_HEIGHT + KEYPAD_SPACING), 9, "9"},

    {KEYPAD_X, KEYPAD_Y + 3 * (KEYPAD_BTN_HEIGHT + KEYPAD_SPACING), -1, "<"},
    {KEYPAD_X + (KEYPAD_BTN_WIDTH + KEYPAD_SPACING),
     KEYPAD_Y + 3 * (KEYPAD_BTN_HEIGHT + KEYPAD_SPACING), 0, "0"},
    {KEYPAD_X + 2 * (KEYPAD_BTN_WIDTH + KEYPAD_SPACING),
     KEYPAD_Y + 3 * (KEYPAD_BTN_HEIGHT + KEYPAD_SPACING), -2, "OK"}};
...
static void drawNumericKeypad() {
  ...
  for (int i = 0; i < 12; i++) {
    ...
    // Button background
    tft->fillRoundRect(btn.x, btn.y, KEYPAD_BTN_WIDTH, KEYPAD_BTN_HEIGHT, 5,
                       TFT_NAVY);
    tft->drawRoundRect(btn.x, btn.y, KEYPAD_BTN_WIDTH, KEYPAD_BTN_HEIGHT, 5,
                       TFT_WHITE);
...
static int getTouchedKeypadButton(int x, int y) {
  for (int i = 0; i < 12; i++) {
    const KeypadButton &btn = keypadButtons[i];
    if (x >= btn.x && x < btn.x + KEYPAD_BTN_WIDTH && y >= btn.y &&
        y < btn.y + KEYPAD_BTN_HEIGHT) {
...
void MenuHidden::update(bool batteryIconPressed) {
  ...
  if (calibState != CalibrationState::NONE) {
    ...
    if (tft != nullptr && tft->getTouch(&tx, &ty)) {
      ...
      if (calibState != CalibrationState::REGEN_ADJUST) { waitTouchRelease(); }
    }
    ...
    // 🔒 v2.9.0: Handle touch calibration
      ...
      if (!TouchCalibration::update()) {
        ...
        if (result.success) {
          ...
          Alerts::play(
              {Audio::AUDIO_ERROR_GENERAL, Audio::Priority::PRIO_HIGH});
        }
        calibState = CalibrationState::NONE;
      }
    }
    // ✅ v2.7.0: Manejar ajuste interactivo de regen
      ...
      if (touched)
        waitTouchRelease(
            DEBOUNCE_SHORT_MS); // Debounce corto después de procesar
    }
    ...
    if (calibState == CalibrationState::NONE && menuActive) { drawMenuFull(); }
    return;
  }
  ...
  if (tft != nullptr && tft->getTouch(&tx, &ty)) {
    ...
    if (touchedOption > 0) {
      ...
      case 3:
        ...
        break; // 🔒 v2.9.0: Touch calibration
      case 4:
        startRegenAdjust();
        break; // ✅ v2.7.0: Ajuste interactivo
      case 5:
        startModulesConfig();
        break; // Configuración interactiva de módulos
      case 6:
        saveAndExit();
        break;
      case 7:
