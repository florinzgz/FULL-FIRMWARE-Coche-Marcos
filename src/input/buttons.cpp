...
static bool readPin(uint8_t pin, int idx) {
  ...
  // Debounce: solo establezco stableState si el valor nuevo se mantiene el
  // tiempo mínimo
  ...
  if (now - lastSignal[idx] >= DEBOUNCE_MS) { stableState[idx] = nowState; }
...
void Buttons::update() {
  ...
  unsigned long now = millis();
  bool state[BTN_COUNT] = {readPin(PIN_BTN_LIGHTS, BTN_LIGHTS)};
  ...
  if (state[BTN_LIGHTS] && !lastState[BTN_LIGHTS]) {
    ...
    if (!longPressTriggered[BTN_LIGHTS] &&
        now - pressStartMs[BTN_LIGHTS] >= LONG_PRESS_MS) {
