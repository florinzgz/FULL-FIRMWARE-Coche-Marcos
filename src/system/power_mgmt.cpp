...
// PIN_RELAY_MAIN (GPIO35), PIN_RELAY_TRAC (GPIO5), PIN_RELAY_DIR (GPIO6),
// PIN_RELAY_SPARE (GPIO7)
// -----------------------
constexpr uint8_t PIN_RELAY_POWER_HOLD =
    PIN_RELAY_MAIN; // Relé 1: Power Hold Buck 5V
constexpr uint8_t PIN_RELAY_AUX_12V =
    PIN_RELAY_TRAC; // Relé 2: 12V Auxiliares (sensores/encoder)
constexpr uint8_t PIN_RELAY_MOTOR_24V =
    PIN_RELAY_DIR; // Relé 3: 24V Potencia (motores)
constexpr uint8_t PIN_RELAY_OPTIONAL =
    PIN_RELAY_SPARE; // Relé 4: Opcional/Seguridad
...
// -----------------------
constexpr uint8_t PIN_POWER_ON =
    PIN_KEY_ON; // GPIO 40: Detección ignition ON (INPUT_PULLUP, LOW=ON)
constexpr uint8_t PIN_SHUTDOWN_REQUEST =
    PIN_KEY_OFF; // GPIO 41: Detección shutdown request (INPUT_PULLUP,
                 // LOW=Shutdown)
...
constexpr uint32_t SHUTDOWN_DELAY_MS =
    5000; // 5 segundos power hold tras apagar llave
 ...
 */
static void activateRelay(uint8_t pin) { digitalWrite(pin, HIGH); }

/**
 * @brief Desactiva un relé (LOW = abierto)
 */
static void deactivateRelay(uint8_t pin) { digitalWrite(pin, LOW); }
...
void init() {
  ...
  // ✅ v2.15.0: Configurar detección dual de alimentación (GPIO 40/41,
  // INPUT_PULLUP)
  pinMode(PIN_POWER_ON, INPUT_PULLUP); // GPIO 40: Ignition ON detection
  pinMode(PIN_SHUTDOWN_REQUEST,
          INPUT_PULLUP); // GPIO 41: Shutdown request detection
  ...
  Logger::info("PowerMgmt: Inicializado - Power Hold y 12V Auxiliares activos "
               "(GPIO 40/41), esperando centrado volante");
}
...
void update() {
  ...
  if (!keyOn && currentState >= PowerState::POWER_HOLD &&
  ...
  case PowerState::POWER_HOLD:
    ...
    // Nota: 12V auxiliares ya está activo desde init() para permitir búsqueda
    // de centro
    if (steeringCentered) {
...
bool isKeyOn() { return readKeyWithDebounce(); }

PowerState getState() { return currentState; }

bool isFullPowerOn() { return currentState == PowerState::FULL_POWER; }
...
bool initOK() { return initialized; }
