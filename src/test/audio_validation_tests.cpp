...
static bool testAllTracksDefinedInEnum() {
  ...
  // Esto se valida en tiempo de compilación, pero podemos verificar algunos
  // tracks clave
  ...
  if (static_cast<uint16_t>(Audio::AUDIO_TRACCION_4X2) != 38)
    allDefined = false;

  // Verificar tracks avanzados (39-68)
  if (static_cast<uint16_t>(Audio::AUDIO_ABS_ACTIVADO) != 39)
    allDefined = false;
...
static bool testQueueOverflow() {
  ...
  for (uint16_t i = 1; i <= 10; i++) {
    if (Audio::AudioQueue::push(i, Audio::PRIO_NORMAL)) { accepted++; }
  }
...
static bool testAllBasicTracks() {
  ...
  for (uint16_t track = 1; track <= 38; track++) {
    ...
    if ((track - 1) % 8 == 0) { Audio::AudioQueue::init(); }
...
static bool testAllAdvancedTracks() {
  ...
  for (uint16_t track = 39; track <= 68; track++) {
    // Reiniciar cola para cada grupo de pruebas
    if ((track - 39) % 8 == 0) { Audio::AudioQueue::init(); }
  ...
  if (failedCount > 0) {
    ...
    for (uint32_t i = 0; i < testCount; i++) {
      if (!testResults[i].passed) {
        Logger::errorf("  - %s: %s", testResults[i].testName,
