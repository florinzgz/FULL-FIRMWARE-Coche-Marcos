...
// Test parameters
static const uint32_t MAX_ACCEPTABLE_FEED_INTERVAL_MS = 8000;  // 80% of 10s timeout
static const uint32_t MIN_FEED_INTERVAL_MS = 50;  // Should feed at least every 50ms
...
bool testFeedInterval() {
    ...
    if (!intervalOk) {
        if (interval > MAX_ACCEPTABLE_FEED_INTERVAL_MS) {
            Logger::errorf("Feed interval too long (%lu ms) - risk of watchdog timeout!", interval);
        } else {
            Logger::warnf("Feed interval very short (%lu ms) - may impact performance", interval);
        }
    }
...
bool testFeedCounting() {
    ...
    Logger::infof("Feed count increased from %lu to %lu", countBefore, countAfter);
