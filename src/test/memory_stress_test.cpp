...
static void recordTest(const char* name, bool passed, uint32_t before, uint32_t after, uint32_t timeMs) {
    if (testCount < MAX_TEST_RESULTS) {
        ...
        if (passed) {
            passedCount++;
            Logger::infof("✅ MEMORY TEST PASSED: %s (delta: %ld bytes, %lums)", 
                         name, (int32_t)(after - before), timeMs);
        } else {
            failedCount++;
            Logger::errorf("❌ MEMORY TEST FAILED: %s (leak: %ld bytes, %lums)", 
                          name, (int32_t)(before - after), timeMs);
        }
    }
}
...
bool testRepeatedInitialization() {
    ...
    // Test 1: Shifter repeated init
    {
        ...
        recordTest("Shifter Repeated Init", passed, heapBefore, heapAfter, duration);
        allPassed &= passed;
    ...
    // Test 2: Current sensors repeated init
    {
        ...
        recordTest("Current Sensors Repeated Init", passed, heapBefore, heapAfter, duration);
        allPassed &= passed;
...
bool testMallocFailures() {
    ...
    Logger::info("Malloc failure handling verified via code review (nullptr checks present)");
    return true;
}
...
bool testHeapFragmentation() {
    ...
    uint32_t startTime = millis();
    uint32_t heapBefore = ESP.getFreeHeap();
    uint32_t largestBlockBefore = heap_caps_get_largest_free_block(MALLOC_CAP_8BIT);
    ...
    uint32_t heapAfter = ESP.getFreeHeap();
    uint32_t largestBlockAfter = heap_caps_get_largest_free_block(MALLOC_CAP_8BIT);
    uint32_t duration = millis() - startTime;
...
void printMemoryStats() {
    ...
    Logger::infof("Free Heap: %lu bytes (%.1f%%)", freeHeap, (freeHeap * 100.0f / totalHeap));
