...
namespace {
constexpr float SPEED_TOLERANCE_KMH =
    0.5f; // Tolerancia de velocidad para considerar vehículo detenido
constexpr int MIN_BATTERY_PERCENT_FOR_OTA =
    50; // Porcentaje mínimo de batería para actualización OTA
} // namespace
...
void MenuWiFiOTA::checkForUpdates() {
  if (!isConnected()) {
    Logger::warn(
        "OTA: No se puede verificar actualizaciones - WiFi no conectado");
    // Audio::playAlert(Audio::ALERT_ERROR);  // Audio module not implemented
    // yet
    return;
  }

  // 🔒 v2.10.2: Implementación básica de verificación de actualizaciones
  // En una implementación completa, esto haría una petición HTTP a GitHub
  // Releases o a un servidor de actualizaciones personalizado.
  //
  ...
  "https://api.github.com/repos/florinzgz/FULL-FIRMWARE-Coche-Marcos/releases/latest";
...
void MenuWiFiOTA::installUpdate() {
  ...
  Logger::info(
      "OTA: Iniciando actualización - Vehículo detenido, PARK, batería OK");
  isUpdating = true;
  // Audio::playAlert(Audio::ALERT_BEEP);  // Audio module not implemented yet
...
bool MenuWiFiOTA::isConnected() { return WiFi.status() == WL_CONNECTED; }

int MenuWiFiOTA::getRSSI() { return WiFi.RSSI(); }

String MenuWiFiOTA::getSSID() { return WiFi.SSID(); }

String MenuWiFiOTA::getIP() { return WiFi.localIP().toString(); }
...
String MenuWiFiOTA::getLatestVersion() { return latestVersion; }