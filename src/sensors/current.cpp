...
// 6 sensores: 0-3 motores tracción (50A), 4 batería (100A), 5 motor dirección
// (50A)
static INA226 *ina[Sensors::NUM_CURRENTS] = {nullptr, nullptr, nullptr,
...
static bool tcaSelect(uint8_t channel) {
  ...
  if (i2cMutex != nullptr &&
    ...
    // Cuando xSemaphoreTake retorna pdFALSE (timeout), el mutex NO fue
    // adquirido, por lo tanto NO hay necesidad de llamar xSemaphoreGive. El
    // mutex sigue disponible para otros tasks.
...
void Sensors::initCurrent() {
  ...
  for (int i = 0; i < NUM_CURRENTS; i++) {
    ...
    ina[i] = new (std::nothrow)
        INA226(0x40); // Address will be selected via TCA9548A multiplexer
    ...
    if (!ina[i]->begin()) {
      ...
      sensorOk[i] = false;
      // NO marcar allOk como falso aquí: fallo no crítico, sistema continúa
      // degradado NO llamar a System::logError() - solo warning El sistema
      // puede continuar sin este sensor específico.
    } else {
  ...
  if (!allOk) {
    Logger::warn(
        "INA226 init: algunos sensores no disponibles - modo degradado");
  }
}
    ...
    if (!sensorOk[i] || !ina[i]) {
      ...
      if (millis() >= state.nextRetryMs) {
        ...
        if (i >= 0 && i < 8) { // TCA9548A has 8 channels (0-7)
          ...
          if (I2CRecovery::reinitSensor(i, 0x40, i) && ina[i] != nullptr &&
              ina[i]->begin()) {
            sensorOk[i] = true;
...
bool Sensors::currentInitOK() { return initialized; }
