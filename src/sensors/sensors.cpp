...
void init() {
  ...
  Logger::info(
      "Sensors: Initializing TOFSense-M S (UART0, 921600 baud, 8x8 matrix)...");
...
void update() {
  // v2.15.0: Update TOFSense-M S obstacle detection (reads UART, parses
  // 400-byte frames)
...
SystemStatus getSystemStatus() {
  ...
  if (cfg.currentSensorsEnabled) {
    for (int i = 0; i < NUM_CURRENTS; i++) {
      if (isCurrentSensorOk(i)) { status.currentSensorsOK++; }
    }
    // Canal de batería
  ...
  if (cfg.tempSensorsEnabled) {
    ...
    for (int i = 0; i < NUM_TEMPS; i++) {
      if (isTemperatureSensorOk(i)) {
        ...
        if (temp > TEMP_CRITICAL_CELSIUS) { status.temperatureWarning = true; }
      }
    }
  }
  ...
  if (cfg.wheelSensorsEnabled) {
    for (int i = 0; i < NUM_WHEELS; i++) {
      if (isWheelSensorOk(i)) { status.wheelSensorsOK++; }
    }
  }
  ...
  status.criticalSensorsOK =
      status.batteryMonitorOK &&
      (status.wheelSensorsOK >= MIN_WHEEL_SENSORS_CRITICAL);

  // Todos los sensores habilitados están OK
  bool currentOK = !cfg.currentSensorsEnabled ||
                   (status.currentSensorsOK == status.currentSensorsTotal);
  bool tempOK = !cfg.tempSensorsEnabled ||
                (status.temperatureSensorsOK == status.temperatureSensorsTotal);
  bool wheelOK = !cfg.wheelSensorsEnabled ||
                 (status.wheelSensorsOK == status.wheelSensorsTotal);
...
bool getSensorDiagnosticText(uint8_t sensorType, uint8_t sensorIdx,
  ...
  case 0: // Corriente (INA226)
    ...
    if (isCurrentSensorOk(sensorIdx)) {
      ...
      snprintf(buffer, bufSize, "INA%d: %.1fA %.1fV OK", sensorIdx, current,
               voltage);
      return true;
    } else {
...
InputDeviceStatus getInputDeviceStatus() {
  ...
  // multimedia y mode4x4 eliminados en v2.14.0 (ahora controlados por touch
  // screen)
