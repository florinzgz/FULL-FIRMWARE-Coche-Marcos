...
// Hardware instances
static HardwareSerial TOFSerial(ObstacleConfig::UART_NUM);  // UART0 for TOFSense-M S
...
static bool validateFrameHeader(const uint8_t* frame) {
    for (uint8_t i = 0; i < ObstacleConfig::HEADER_LENGTH; i++) {
        if (frame[i] != ObstacleConfig::FRAME_HEADER[i]) {
            return false;
        }
    }
    return true;
...
// Returns distance in millimeters (negative values indicate invalid/out of range)
static int16_t parsePixelDistance(const uint8_t* pixelData) {
    // Extract 3 bytes in little-endian format
    int32_t temp = (int32_t)(pixelData[0] | (pixelData[1] << 8) | (pixelData[2] << 16));
    ...
    // Convert: divide by 256, then divide by 1000 to get meters, then *1000 for mm
...
// Update sensor data with new distance reading (legacy function for compatibility)
            ...
            if (bufferIndex >= ObstacleConfig::FRAME_LENGTH) {
                ...
                if (parseFrame()) {
                    ...
                    if (now - lastLogMs > ObstacleConfig::LOG_INTERVAL_MS) {
                        Logger::infof("TOFSense 8x8: Min distance = %u mm (%d valid pixels)", 
                                     sensor.minDistance,
                                     countValidZones());
                        lastLogMs = now;
                    }
