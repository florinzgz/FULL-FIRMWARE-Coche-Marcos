...
void Alerts::play(const Audio::Item &item) {
  ...
  if (item.track == 0 || item.track > 68) {
    Logger::warnf("Alerts play(item): track inválido (%u). Rango válido: 1-68",
                  (unsigned)item.track);
  ...
  Logger::infof("Alerts queued track %u prio %u", (unsigned)item.track,
                (unsigned)item.prio);
}
...
void Alerts::play(Audio::Track t) {
  ...
  if (trackId == 0 || trackId > 68) {
    Logger::warnf("Alerts play(Track): track inválido (%u). Rango válido: 1-68",
                  (unsigned)trackId);
    System::logError(721);
    return;
  }
...
bool Alerts::initOK() { return initialized; }