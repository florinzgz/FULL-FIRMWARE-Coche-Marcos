...
static HardwareSerial DFSerial(2);  // UART2 for DFPlayer (v2.12.0: migrado de UART0)

// Flag de inicialización
static bool initialized = false;

void Audio::DFPlayer::init() {
    // Initialize Serial port for DFPlayer on UART2 pins (GPIO 15/16)
    // ✅ v2.12.0: Migrado de UART0 (GPIO 43/44) a UART2 (GPIO 15/16)
    // - Libera UART0 para debug/USB Serial sin interferencias
    // - GPIO 15 = TX (envía comandos al DFPlayer)
    // - GPIO 16 = RX (recibe respuestas del DFPlayer)
    ...
    if (!ok) {
        Logger::errorf("DFPlayer init failed on UART2");
        System::logError(700); // código reservado: fallo init
        initialized = false;
        return;
    }
    Logger::info("DFPlayer initialized on UART2 (GPIO15/16)");
    initialized = true;
}
