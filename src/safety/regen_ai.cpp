...
static const float EFFICIENCY_LOOKUP[5][4] = {{40.0f, 45.0f, 50.0f, 55.0f},
...
static void updateFeatures() {
  ...
  state.currentFeatures.speed =
      (validCount > 0) ? (speedSum / validCount) : 0.0f;
  ...
  if (now - lastSpeedTime > 0) {
    ...
    float dv =
        (state.currentFeatures.speed - lastSpeed) / 3.6f; // Convert km/h to m/s
  ...
  state.currentFeatures.batterySOC =
      constrain(((voltage - 36.0f) / (42.0f - 36.0f)) * 100.0f, 0.0f, 100.0f);
...
void update() {
  ...
  if (state.currentFeatures.batteryTemp > config.tempMax) { canRegen = false; }

  // Only regen when decelerating
  if (state.currentFeatures.acceleration >= -0.1f) { canRegen = false; }

  // Minimum speed for regen
  if (state.currentFeatures.speed < 5.0f) { canRegen = false; }
  ...
  state.prediction = {.regenPower = predictRegenPower(state.currentFeatures),
                      .efficiency = 0.0f,
                      .confidence = 0.8f};
  ...
  if (state.active) { state.cycleCount++; }
...
void evaluateRegen() { update(); }

float getOptimalRegenPower() {
  if (!initialized || !config.enabled || !state.active) { return 0.0f; }
...
bool RegenAI::initOK() { return initialized; }
