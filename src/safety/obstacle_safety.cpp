...
// Parking assist, collision avoidance, blind spot warning, adaptive cruise
// control v2.12.0: Intelligent 5-zone obstacle avoidance with child reaction
// detection Author: Copilot AI Assistant Date: 2025-12-23 Note: Placeholder
// implementation - VL53L5X sensors not yet integrated
...
// Alert throttling
static constexpr uint32_t ALERT_INTERVAL_MS =
    1000; // 1 second between same alerts
...
void init() {
  Logger::info(
      "Obstacle safety v2.12.0: Initializing with 5-zone detection...");
...
void update() {
  ...
  // 🔒 CRITICAL SAFETY: If sensors are unhealthy (timeout/error), apply
  // emergency brake This is fail-safe behavior - if we lose sensor data, assume
  // danger and stop
  if (obstStatus.sensorsHealthy == 0) {
    if (config.collisionAvoidanceEnabled) {
      ...
      if (now - lastSensorFailAlertMs >
          ALERT_INTERVAL_MS * 3) { // Every 3 seconds
  ...
  if (frontDist >= ::ObstacleConfig::DISTANCE_INVALID)
    frontDist = ::ObstacleConfig::DISTANCE_MAX;
  if (rearDist >= ::ObstacleConfig::DISTANCE_INVALID)
    rearDist = ::ObstacleConfig::DISTANCE_MAX;

  // Find closest obstacle
  uint16_t minDist = ::ObstacleConfig::DISTANCE_MAX;
  if (frontDist < minDist) {
    minDist = frontDist;
    state.closestObstacleSensor = 0;
  }
  if (rearDist < minDist) {
    minDist = rearDist;
    state.closestObstacleSensor = 1;
  }
  ...
  if (config.collisionAvoidanceEnabled) {
    ...
    case 4: // ZONE 4: Critical (20-50cm) - Forced reduction (10-40%)
      ...
      state.speedReductionFactor =
          0.1f + (minDist - ZONE_5_THRESHOLD) * 0.3f /
                     (ZONE_4_THRESHOLD - ZONE_5_THRESHOLD);
      state.speedReductionFactor =
          constrain(state.speedReductionFactor, 0.1f, 0.4f);
      if (now - lastAlertMs[0] > ALERT_INTERVAL_MS) {
        Alerts::play(Audio::AUDIO_EMERGENCIA);
        Logger::warnf("ZONE 4: CRITICAL! Distance=%dmm, factor=%.2f", minDist,
                      state.speedReductionFactor);
        lastAlertMs[0] = now;
      }
      break;
    ...
    case 3: // ZONE 3: Warning (50-100cm) - Reaction-based (40-70%)
      if (state.childReactionDetected) {
        ...
        state.speedReductionFactor =
            0.4f + (minDist - ZONE_4_THRESHOLD) * 0.3f /
                       (ZONE_3_THRESHOLD - ZONE_4_THRESHOLD);
        state.speedReductionFactor =
            constrain(state.speedReductionFactor, 0.4f, 0.7f);
      }
      if (now - lastAlertMs[0] > ALERT_INTERVAL_MS) {
    ...
    case 2: // ZONE 2: Caution (100-150cm) - Gentle brake if no reaction
            // (85-100%)
      if (!state.childReactionDetected) {
        // No reaction - gentle brake (regardless of ACC priority)
        state.speedReductionFactor =
            0.85f + (minDist - ZONE_3_THRESHOLD) * 0.15f /
                        (ZONE_2_THRESHOLD - ZONE_3_THRESHOLD);
        state.speedReductionFactor =
            constrain(state.speedReductionFactor, 0.85f, 1.0f);
      } else {
  ...
  if (config.parkingAssistEnabled && zone == 0) {
    if (frontDist < config.parkingBrakeDistanceMm) {
      ...
      state.speedReductionFactor =
          std::min(state.speedReductionFactor, parkingFactor);
...
void getState(SafetyState &st) { st = state; }

void getConfig(SafetyConfig &cfg) { cfg = config; }
...
bool isCollisionImminent() { return state.collisionImminent; }

bool isParkingAssistActive() { return state.parkingAssistActive; }

bool isBlindSpotActive() { return state.blindSpotLeft || state.blindSpotRight; }

float getSpeedReductionFactor() { return state.speedReductionFactor; }

void enableParkingAssist(bool enable) { config.parkingAssistEnabled = enable; }

void enableCollisionAvoidance(bool enable) {
  config.collisionAvoidanceEnabled = enable;
}

void enableBlindSpot(bool enable) { config.blindSpotEnabled = enable; }
