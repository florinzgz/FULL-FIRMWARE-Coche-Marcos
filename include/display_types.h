...
// Constante para cache de estado no inicializado (usado en HUD/icons para
...
struct CarData {
  ...
  float
      voltage; // Voltaje batería 24V (INA226 canal 4) - alias de batteryVoltage
  ...
  float temperature;    // Temperatura principal (motor/pedal) - alias de
                        // motorTemp[0]
  ...
  // Pedal y dirección
  float pedalPosition;   // Posición pedal acelerador (0-100%) - alias de
                         // throttlePercent
  float throttlePercent; // Posición pedal acelerador (0-100%)
  float pedalPercent; // Porcentaje pedal calculado (0-100%) - uso en HUD, -1 si
                      // inválido
