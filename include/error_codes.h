...
constexpr uint16_t SHIFTER_NOT_INITIALIZED = 651;
constexpr uint16_t SHIFTER_NOT_IN_PARK = 652;
constexpr uint16_t SHIFTER_INVALID_STATE = 653;
...
inline const char* getErrorDescription(uint16_t code) {
    ...
    if (code == SHIFTER_INVALID_STATE) return "Palanca estado invalido";
