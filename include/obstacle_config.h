...
// - Sensor LiDAR con matriz 8x8 conectado por UART0 (921600 baud, pines nativos)
...
namespace ObstacleConfig {
    ...
    constexpr uint32_t UART_BAUDRATE = 921600;      // Baudrate del TOFSense-M S (8x8 mode)
    ...
    //   Each pixel: 3 bytes distance + 1 byte signal + 1 byte status + 1 byte reserved
    ...
    constexpr uint8_t BYTES_PER_PIXEL = 6;          // 3 distance + 1 signal + 1 status + 1 reserved
    ...
    constexpr uint8_t POS_RESERVED = 396;           // Reserved (4 bytes)
    ...
    constexpr uint16_t DISTANCE_MAX = 4000;         // Max detection range (4m for TOFSense-M S)
    constexpr uint16_t DISTANCE_INVALID = UINT16_MAX; // Invalid distance marker
    
    // Timing configuration
    constexpr uint32_t UPDATE_INTERVAL_MS = 66;     // 15Hz update rate (matches sensor output)
    constexpr uint32_t MEASUREMENT_TIMEOUT_MS = 500; // Max measurement time
    constexpr uint32_t UART_READ_TIMEOUT_MS = 200;  // UART read timeout (increased for large frames)
