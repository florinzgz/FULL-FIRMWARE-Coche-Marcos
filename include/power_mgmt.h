 ...
 * - GPIO 40 (PIN_KEY_ON): Detección ignition ON (INPUT_PULLUP, LOW=ON,
 * HIGH=OFF)
 * - GPIO 41 (PIN_KEY_OFF): Detección shutdown request (INPUT_PULLUP,
 * LOW=Shutdown, HIGH=Normal)
 ...
 * @return true si ignition ON (GPIO 40 LOW) y no hay shutdown request (GPIO 41
 * HIGH)
